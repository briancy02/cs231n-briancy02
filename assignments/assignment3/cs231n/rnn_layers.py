...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(1, x.shape[1]):
      h[:,t,:] = rnn_step_forward(x[:,t,:], h[:,t-1,:], Wx, Wh, b)[0]
      cache[t] = rnn_step_forward(x[:,t,:], h[:,t-1,:], Wx, Wh, b)[1]
...
def rnn_backward(dh, cache):
    ...
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    dx = np.zeros((cache[0]["x"].shape[0], dh.shape[1], cache[0]["x"].shape[1]))
    dnext_h = np.zeros_like(cache[0]["prev_h"])
    dWx = np.zeros((cache[0]["Wx"].shape[0], dh.shape[1], cache[0]["Wx"].shape[1]))
    dWh = np.zeros((cache[0]["Wh"].shape[0], dh.shape[1], cache[0]["Wh"].shape[1]))
    db = np.zeros((dx.shape[1], dWh.shape[0]))
    dx[:,dh.shape[1] - 1,:], dnext_h, dWx[:,dh.shape[1] - 1,:], dWh[:,dh.shape[1] - 1,:], db[dh.shape[1] - 1] = rnn_step_backward(dh[:,dh.shape[1] - 1,:], cache[dh.shape[1] - 1]) 
    for t in range(dh.shape[1] - 2, -1, -1):
      dx[:,t,:], dnext_h, dWx[:,t,:], dWh[:,t,:], db[t] = rnn_step_backward(dh[:,t,:] + dnext_h, cache[t]) 
    dWh = dWh.sum(axis=1)
    dWx = dWx.sum(axis=1)
    db = db.sum(axis=0)
    dh0 = dnext_h
...
def word_embedding_backward(dout, cache):
    ...
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    dW = np.zeros_like(cache["W"])
    for n in range(dout.shape[0]):
      np.add.at(dW, cache[n], dout[n,:,:])
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    i = sigmoid(out[:,:Wh.shape[0]])
    f = sigmoid(out[:,Wh.shape[0]:2*Wh.shape[0]])
    o =  sigmoid(out[:,Wh.shape[0]*2:3*Wh.shape[0]])
    temp_c = np.tanh(out[:,Wh.shape[0]*3:4*Wh.shape[0]])
    next_c = prev_c * f + temp_c * i
    next_h = o * np.tanh(next_c)

    cache = (x, Wx, prev_h, Wh, i, f, o, temp_c, next_c, prev_c, next_h, out)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x, Wx, prev_h, Wh, i, f, o, temp_c, next_c, prev_c, next_h, out = cache
    ...
    df = dnext_c * prev_c
    di = dnext_c * temp_c
    dout[:,Wh.shape[0]*3:4*Wh.shape[0]] = dtemp_c * (1 - np.tanh(out[:,Wh.shape[0]*3:4*Wh.shape[0]])**2)
    dout[:,:Wh.shape[0]] = di * sigmoid(out[:,:Wh.shape[0]]) * (1 - sigmoid(out[:,:Wh.shape[0]]))
    dout[:,Wh.shape[0]:2*Wh.shape[0]] = df * sigmoid(out[:,Wh.shape[0]:2*Wh.shape[0]]) * (1 - sigmoid(out[:,Wh.shape[0]:2*Wh.shape[0]]))
    dout[:,Wh.shape[0]*2:3*Wh.shape[0]] = do * sigmoid(out[:,Wh.shape[0]*2:3*Wh.shape[0]]) * (1 - sigmoid(out[:,Wh.shape[0]*2:3*Wh.shape[0]]))
    dWx = x.T.dot(dout)
